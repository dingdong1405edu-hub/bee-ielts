...
export default async function PodcastsListPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 py-6 sm:py-8 space-y-6 sm:space-y-8">
      {/* Hero */}
      <section className="relative overflow-hidden rounded-3xl border-2 border-primary/40 gradient-brand p-6 sm:p-8 text-white shadow-lg">
        <div className="absolute -right-12 -top-12 h-48 w-48 rounded-full bg-white/10 blur-2xl pointer-events-none" />
        <div className="absolute -left-8 -bottom-8 h-40 w-40 rounded-full bg-white/15 blur-2xl pointer-events-none" />
      ...
      {/* Grid */}
      {episodes.length === 0 ? (
        <div className="rounded-2xl border-2 border-dashed border-primary/40 bg-cream/50 p-10 text-center">
          <Headphones className="mx-auto h-12 w-12 text-primary mb-3" />
        ...
        <div className="grid gap-4 sm:gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {episodes.map((ep) => (
            <Link
              key={ep.id}
              href={`/podcasts/${ep.id}`}
              className="group relative overflow-hidden rounded-2xl bg-paper border-2 border-primary/20 shadow-sm hover:shadow-lg hover:border-primary/60 transition-all hover:-translate-y-0.5"
            >
              ...
              <div className="relative aspect-video overflow-hidden bg-muted">
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <div className="grid h-14 w-14 place-items-center rounded-full bg-primary text-primary-foreground shadow-lg ring-4 ring-white/30">
              ...
              <div className="p-4 space-y-1.5">
                <h2 className="font-display font-bold text-base leading-tight line-clamp-2 group-hover:text-primary transition-colors">
                  {ep.title}
                </h2>
