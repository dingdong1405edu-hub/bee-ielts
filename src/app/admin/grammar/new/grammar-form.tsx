...
type Ex = { prompt: string; answer: string };
type Lesson = { title: string; order: number; content: string; exercises: Ex[] };

const blankEx = (): Ex => ({ prompt: "", answer: "" });
const blankLesson = (order: number): Lesson => ({
  title: "",
  order,
  content: "",
  exercises: [blankEx()],
...
export function GrammarForm({ units }: { units: { id: string; title: string; level: string }[] }) {
  ...
  const submit = async () => {
    ...
    for (const l of lessons) {
      ...
      if (!l.content.trim()) return toast.error("Mỗi bài học cần có nội dung lý thuyết");
  ...
  return (
    <div className="max-w-3xl space-y-4">
      <h1 className="text-2xl font-bold">Thêm bài học Grammar</h1>
      ...
      {lessons.map((lesson, li) => (
        <Card key={li}>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <Label>Nội dung lý thuyết</Label>
              <Textarea
                value={lesson.content}
                onChange={(e) => patchLesson(li, { content: e.target.value })}
                className="min-h-[120px]"
                placeholder="Giải thích ngữ pháp. Dùng **chữ đậm** để nhấn mạnh."
              />
            </div>
            ...
            {lesson.exercises.map((ex, ei) => (
              <div key={ei} className="rounded-lg border p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-sm">Câu {ei + 1} (điền vào chỗ trống)</span>
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder="VD: She ___ from Vietnam."
                  />
                </div>
                <div>
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => patchLesson(li, { exercises: [...lesson.exercises, blankEx()] })}
            >
      ...
      <div className="flex gap-2 pt-2">
        <Button variant="outline" onClick={() => router.push("/admin/grammar")}>
          Huỷ
        </Button>
